...
public class TowerOfHanoi {
    ...
    void moveDisksBetweenTwoPoles(Stack src, Stack dest, 
                                            char s, char d) 
    {
    	int srcTopDisc = pop(src);
    	int destTopDisc = pop(dest);
    	
    	// if src is empty
    	if(srcTopDisc == Integer.MIN_VALUE){
    		push(src, destTopDisc);
    		moveDisk(d, s, destTopDisc);
    	}
    	//if dest is empty.
    	else if(destTopDisc == Integer.MIN_VALUE){
    		push(dest, srcTopDisc);
    		moveDisk(s, d, srcTopDisc);
    	}
    	// if des-disc > src-disc
    	else if(srcTopDisc > destTopDisc){
    		push(src, srcTopDisc);
    		push(src, destTopDisc);
    		moveDisk(d, s, destTopDisc);
    	}else{
    		push(dest, destTopDisc);
    		push(dest, srcTopDisc);
    		moveDisk(s, d, srcTopDisc);
